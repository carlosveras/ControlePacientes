...
$(document).ready(function () {
    ...
    tablaproducto = $('#tbProducto').DataTable({
        "ajax": {
            ...
            "url": $.MisUrls.url._ObtenerProductoStockPorTienda + "1",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
    ...
    tablaformapagamento = $('#tbformapagamento').DataTable({
        "ajax": {
        ...
        "columns": [
            {
                "data": "IdFormaPagto", "render": function (data, type, row, meta) {
                    return "<button class='btn btn-sm btn-primary ml-2' type='button' onclick='formapagamentoSelect(" + JSON.stringify(row) + ")'><i class='fas fa-check'></i></button>"
                },
                "orderable": false,
                "searchable": false,
                "width": "90px"
...
// tablaproducto.ajax.url($.MisUrls.url._ObtenerProductoStockPorTienda + "?IdTienda=" + parseInt($("#txtIdTienda").val()) ).load();
    tablaproducto.ajax.url($.MisUrls.url._ObtenerProductoStockPorTienda + "1").load();
...
$('#btnBuscarFormaPagamento').on('click', function () {

    tablaformapagamento.ajax.reload();
...
function formapagamentoSelect(json) {
    $("#txtIdFormaPagto").val(json.IdFormaPagto);
    $("#txtformapagamento").val(json.Descricao);
...
$('#btnTerminarGuardarVenta').on('click', function () {
    ...
    if ($("#txtclientedocumento").val().trim() == "" || $("#txtclientenombres").val().trim() == "") {
        swal("Mensagem", "Complete os dados do cliente", "warning");
        return;
    }
    ...
    VENTA = "<VENTA>" +
        ...
        "<IdFormaPagto>" + $("#txtIdFormaPagto").val() + "</IdFormaPagto>" +
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data.estado) {
                ...
                //CLIENTE
                $("#cboclientetipodocumento").val("CPF");
